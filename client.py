...
OPENAI_API_KEY = "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"
GOOGLE_API_KEY = "AIzaSyCTxaIW3pn7arF8XlpuVjjN9F_ZuqP7Lmw"
GOOGLE_CX = "377ea0812fa5e47de"
...
# Function to call the API tools
def call_api_tool(tool_name, data):
    ...
    try:
        ...
        if response.status_code != 200:
            error_messsage = f"Error {response.status_code} from server:{response.test}"
...
# set page config
st.set_page_config(
    page_title="Assignment_Grader",
    page_icon = "📝",
    layout= "wide"
)

# Main Title
st.title("📝 Assignment_Grader")
...
with st.sidebar.expander("Server settings", expanded=True):
    ...
    if st.button("Save Server URL"):
        ...
        st.success(f"✅ Server url uploaded to {server_url}")
...
with tab1:
    st.header("Upload the Assignment")
    ...
    if uploaded_file is not None:
        ...
        if st.button("Process Document"):
            with st.spinner("Processing document...."):
                ...
                if result is None:
                    st.error("Failed to process document. Check the server connection.")
                elif isinstance(result, str):
                    # if the result is string, is's is either the document text or an error message.
...
with tab2:
    ...
    rubric_templates = {
        "Default Academic": """
        Content (40%): The assignment should demonstrate a thorough understanding of the topic.
        ...
        """,
        "Technical Report": """
        Accuracy (35%): Technical details should be accurate and well-explained.
        ...
        """,
        "Creative Writing": """
        Originality (30%): The work should show creative and original thinking.
    ...
    # Ge default value based on selection
    ...
    if all(k in st.session_state for k in ['file_name']):
        ...
        with col1:
            if 'grade_results' in st.session_state and st.session_state['grade_results'] is not None:
                if isinstance(st.session_state['grade_results'], dict):
                    ...
                    # if it is not a dict just display the raw result
                    grade = str(st.session_state['grade_results'])
                    
                # Display grade in large format
                st.markdown(f"## Grade: {grade}")
                
                # Generate visual indicator based on the grade
                try:
        ...
        if 'plagiarism_results' in st.session_state and st.session_state['plagiarism_results']:
            ...
            if results is None:
                ...
                if isinstance(results, dict):
                    ...
                    st.json(results) # display raw results if the format is unknown
        ...
        with col1:
            if st.button("📥 Export to PDF"):
                st.info("Crating the PDF report...")
                # This is where we have to implement the pdf export
                st.download_button(
                    label="Download PDF",
        ...
        with col2:
            if st.button("💾 Save to Database"):
                ...
                # This is where we have to implement database save
                time.sleep(1)
